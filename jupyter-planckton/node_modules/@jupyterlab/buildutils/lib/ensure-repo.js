...
function ensureBranch() {
    ...
    let files = (0, child_process_1.execSync)('git ls-tree -r HEAD --name-only')
        .toString()
        .trim()
        .split(/\r?\n/);
 ...
 */
function getCoreData(corePaths) {
 ...
 */
function ensureCorePackage(corePackage, corePaths) {
 ...
 */
function ensureFederatedExample() {
    const basePath = path.resolve('.');
    const corePath = path.join(basePath, 'examples', 'federated', 'core_package', 'package.json');
    ...
    coreData.forEach((data, name) => {
        ...
        const keep = (meta === null || meta === void 0 ? void 0 : meta.extension) || (meta === null || meta === void 0 ? void 0 : meta.mimeExtension);
        if (!keep) {
            return;
        }
...
function ensureJupyterlab() {
    ...
    ensureCorePackage(corePackage, corePaths.filter(p => !/\/packages\/testing$/.test(p)));
...
async function ensureIntegrity() {
    ...
    for (const name in locals) {
        ...
        const pkgMessages = await (0, ensure_package_1.ensurePackage)(options);
        if (pkgMessages.length > 0) {
            messages[name] = pkgMessages;
        }
    }
    // ensure the icon svg imports
    pkgMessages = await (0, ensure_package_1.ensureUiComponents)(pkgPaths['@jupyterlab/ui-components']);
    if (pkgMessages.length > 0) {
