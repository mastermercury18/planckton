...
Docs-first Qiskit RAG system with a HARD retrieval gate.

Goal:
- Prefer the CURRENT APIs described in the documentation corpus built from
  qiskit_official_links.txt (the allowlist).
- In STRICT mode (docs-covered): respond ONLY using retrieved docs context.
- In FALLBACK mode (docs-not-covered): allow general model knowledge.

Key features:
- Allowlist enforcement (only URLs from qiskit_official_links.txt)
- Section-level chunking (better precision than page-level retrieval)
- Retrieval confidence gate (STRICT / AMBIGUOUS / FALLBACK)
- Prompt structure: stable system prompt + docs context message + user message
- Optional post-generation deprecated-import linting / rewriting

Usage:
  python3 qiskit_rag_system_linklist_v2.py

In your app:
  rag = QiskitRAGSystem(
      docs_file="comprehensive_qiskit_docs.json",
      allowlist_file="qiskit_official_links.txt"
  )
  handler = EnhancedPlancktonHandler(rag)
  handler.process_message("How do I create a noise model for AerSimulator?", history=[])
...
class QiskitRAGSystem:
    ...
    def _load_docs(self) -> None:
        ...
        if self.allowlist:
            docs = [d for d in docs if d.get("url") in self.allowlist]
    ...
    def _build_chunks(self) -> None:
        ...
        self.chunks = chunks
        print(f"Built {len(self.chunks)} section-level chunks")
    ...
    def _expand_query(self, query: str) -> str:
        ...
        if "aer" in ql or "simulator" in ql:
            expanded += " AerSimulator qiskit_aer noise model backend simulation"
        if "noise" in ql:
            expanded += " NoiseModel depolarizing_error thermal_relaxation_error pauli_error amplitude_damping_error"
    ...
    def doc_coverage(self, query: str, top_k: int = 8) -> Tuple[float, List[Dict]]:
        results = self.search(query, top_k=top_k)
        if not results:
            return 0.0, []
        best = max(r["relevance_score"] for r in results)
        return best, results

    def decide_mode(self, query: str) -> Tuple[str, float, List[Dict]]:
        score, results = self.doc_coverage(query)
    ...
    def get_context_for_query(self, query: str, max_chars: int = 4500, top_k: int = 6) -> str:
...
class EnhancedPlancktonHandler:
    ...
    def _build_messages(self, user_message: str, history: List[Dict]) -> Tuple[List[Dict], str, float]:
        mode, score, _ = self.rag_system.decide_mode(user_message)
...
def test_rag_system() -> None:
    ...
    test_queries = [
        ...
        "How do I use the Sampler primitive?",
        "What is VQE in Qiskit?",
        "How do I transpile a circuit for a backend?",
    ]

    for q in test_queries:
        mode, score, _ = rag.decide_mode(q)
